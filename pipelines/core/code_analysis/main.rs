...
fn call_zsei_query(input: &serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    match action {
        ...
        "SearchWithCode" => {
            // Extended search that returns code content for similarity comparison
            let query = input.get("query").and_then(|q| q.as_str()).unwrap_or("");
            let index_path = format!("{}/indices/code_index.json", zsei_data_path);
                ...
                if let Ok(index) = serde_json::from_str::<serde_json::Value>(&content) {
                    if let Some(containers) = index.get("containers").and_then(|c| c.as_array()) {
                        for container in containers {
                            ...
                            if matches {
                                // Load full container to get code content
                                if let Some(cid) =
                                    container.get("container_id").and_then(|c| c.as_u64())
                                {
                                    let container_path =
                                        format!("{}/local/{}.json", zsei_data_path, cid);
                                    if let Ok(full_content) =
                                        std::fs::read_to_string(&container_path)
                                    {
                                        if let Ok(full_container) =
                                            serde_json::from_str::<serde_json::Value>(&full_content)
                                        {
                                            results.push(full_container);
                                        }
                                    }
                                }
                            }
            ...
            Ok(serde_json::json!({"success": true, "containers": results}))
        }
...
fn call_zsei_write(input: &serde_json::Value) -> Result<serde_json::Value, String> {
    ...
    std::fs::write(
        &container_path,
        serde_json::to_string_pretty(&container_data).unwrap(),
    )
...
fn detect_language(code: &str, file_path: Option<&str>) -> String {
    ...
    if code.contains("interface ") && code.contains(": string") {
...
// ========== FIX #1: Function Calls Extraction ==========

/// Extract function calls from a function body using tree-sitter with regex fallback
fn extract_function_calls(code: &str, language: &str, function_body: &str) -> Vec<String> {
    use tree_sitter::{Language, Parser};

    let mut parser = Parser::new();

    // Set language for tree-sitter
    let lang: Option<Language> = match language.to_lowercase().as_str() {
    ...
    if let Some(lang) = lang {
        if parser.set_language(&lang).is_ok() {
            if let Some(tree) = parser.parse(function_body, None) {
                let root = tree.root_node();
                let mut calls = Vec::new();

                // Walk the tree to find call expressions
                collect_calls_recursive(root, function_body.as_bytes(), &mut calls, language);
    ...
    fallback_extract_calls(function_body, language)
}

/// Recursively collect function calls from tree-sitter AST
fn collect_calls_recursive(
    ...
    // Check for call expressions based on language
    let is_call = match language.to_lowercase().as_str() {
        "rust" => kind == "call_expression" || kind == "macro_invocation",
        "python" => kind == "call",
        "javascript" | "typescript" => kind == "call_expression" || kind == "new_expression",
        "go" => kind == "call_expression",
        _ => kind.contains("call"),
    };

    if is_call {
        // Get the function name from the call
        let func_name = extract_call_name(node, source, language);
        if let Some(name) = func_name {
            let clean_name = name.trim().to_string();
            if !clean_name.is_empty() && !calls.contains(&clean_name) {
    ...
    for i in 0..node.child_count() {
        if let Some(child) = node.child(i) {
            collect_calls_recursive(child, source, calls, language);
        }
    }
}

/// Extract the function name from a call node
fn extract_call_name(node: tree_sitter::Node, source: &[u8], language: &str) -> Option<String> {
    // Try different field names used by different languages
    let func_node = node
        ...
        .or_else(|| {
            // For some languages, the first child is the function name
    ...
    if let Some(func_node) = func_node {
        if let Ok(text) = func_node.utf8_text(source) {
            // Clean up the name - remove arguments, generics, etc.
            let name = text
                .split('(')
                .next()
                .unwrap_or(text)
            ...
            // For method calls like `self.method`, extract just `method`
            let final_name = if name.contains('.') {
                name.rsplit('.').next().unwrap_or(name)
            } else if name.contains("::") {
                // For Rust paths like `module::function`, keep the last part
                name.rsplit("::").next().unwrap_or(name)
            } else {
                name
            };

            return Some(final_name.to_string());
        }
    }
...
/// Fallback regex-based call extraction when tree-sitter isn't available
fn fallback_extract_calls(code: &str, language: &str) -> Vec<String> {
    let mut calls = Vec::new();

    // Common patterns for function calls by language
    ...
    // Keywords and common false positives to skip
    let skip_words: HashSet<&str> = [
        "if", "for", "while", "match", "switch", "case", "return", "fn", "function", "def",
        "class", "struct", "impl", "trait", "let", "const", "var", "mut", "pub", "async", "await",
        "try", "catch", "finally", "throw", "raise", "except", "import", "from", "use", "include",
        "require", "true", "false", "null", "None", "nil",
    ]
    .iter()
    .cloned()
    .collect();
        ...
        if let Ok(re) = regex::Regex::new(pattern) {
            for caps in re.captures_iter(code) {
                if let Some(name_match) = caps.get(1) {
                    let call = name_match.as_str().to_string();
                    if !skip_words.contains(call.as_str()) && !calls.contains(&call) {
                        calls.push(call);
                    }
                }
            }
...
/// Extract the body of a function from code given start and end lines
fn extract_function_body(code: &str, line_start: u32, line_end: u32) -> String {
...
// ========== FIX #2: Similarity Computation ==========

/// Compute actual similarity between two code snippets
fn compute_similarity(code1: &str, code2: &str) -> f32 {
    // Method 1: Token-based Jaccard similarity
    let tokens1: HashSet<&str> = code1
        .split(|c: char| !c.is_alphanumeric() && c != '_')
        .filter(|s| !s.is_empty() && s.len() > 1)
        .collect();

    let tokens2: HashSet<&str> = code2
        .split(|c: char| !c.is_alphanumeric() && c != '_')
        .filter(|s| !s.is_empty() && s.len() > 1)
        .collect();
    ...
    // Method 2: Structure similarity (function/class count)
    let struct1 = count_structures(code1);
    let struct2 = count_structures(code2);

    let struct_sim = if struct1.total > 0 || struct2.total > 0 {
        let common =
            struct1.functions.min(struct2.functions) + struct1.classes.min(struct2.classes);
        let total = struct1.total.max(struct2.total);
        common as f32 / total as f32
    } else {
        1.0 // Both empty = identical
    };

    // Method 3: Line-based similarity (normalized)
    let lines1: Vec<&str> = code1
        .lines()
        .map(|l| l.trim())
        .filter(|l| !l.is_empty() && !l.starts_with("//") && !l.starts_with("#"))
        .collect();
    let lines2: Vec<&str> = code2
        .lines()
        .map(|l| l.trim())
        .filter(|l| !l.is_empty() && !l.starts_with("//") && !l.starts_with("#"))
        .collect();

    let lines1_set: HashSet<&str> = lines1.iter().cloned().collect();
    let lines2_set: HashSet<&str> = lines2.iter().cloned().collect();

    let common_lines = lines1_set.intersection(&lines2_set).count();
    ...
    // Method 4: Import similarity
    let imports1: Vec<&str> = code1
        .lines()
        .filter(|l| {
    ...
    let imports1_set: HashSet<&str> = imports1.iter().cloned().collect();
    let imports2_set: HashSet<&str> = imports2.iter().cloned().collect();

    let common_imports = imports1_set.intersection(&imports2_set).count();
    let total_imports = imports1.len().max(imports2.len());

    let import_sim = if total_imports > 0 {
        common_imports as f32 / total_imports as f32
    } else {
        1.0
    };
    ...
    let similarity =
        (jaccard * 0.35) + (struct_sim * 0.25) + (line_sim * 0.25) + (import_sim * 0.15);

    // Clamp to 0-1 range
    similarity.max(0.0).min(1.0)
}

/// Structure count for similarity comparison
...
fn count_structures(code: &str) -> StructureCount {
    let lines: Vec<&str> = code.lines().collect();
...
// ========== FIX #3: Reverse Dependencies ==========

/// File info for reverse dependency computation
#[derive(Debug, Clone)]
struct FileInfoForDeps {
    container_id: u64,
    file_path: String,
    imports: Vec<String>,
}

/// Compute reverse dependencies (what depends on this file)
fn compute_reverse_dependencies(file_path: &str, all_files: &[FileInfoForDeps]) -> Vec<u64> {
    let mut depended_by = Vec::new();

    // Get the module/file name variations to search for
    let path_obj = Path::new(file_path);
    let file_name = path_obj.file_stem().and_then(|s| s.to_str()).unwrap_or("");

    // Also compute the module path (for imports like "crate::module::submodule")
    ...
    // Get the file name without path for simpler matching
    let file_name_with_ext = path_obj.file_name().and_then(|s| s.to_str()).unwrap_or("");

    // Search all other files for references to this file
    for other_file in all_files {
        if other_file.file_path == file_path {
            continue; // Skip self
        }

        // Check if any of this file's imports reference our file
        let depends = other_file.imports.iter().any(|imp| {
            let imp_lower = imp.to_lowercase();
            let file_name_lower = file_name.to_lowercase();
            let module_path_lower = module_path.to_lowercase();

            imp_lower.contains(&file_name_lower)
                || imp_lower.contains(&module_path_lower)
                || imp.ends_with(file_name)
                || imp.ends_with(&file_name_with_ext.replace('.', ""))
        });

        if depends {
            depended_by.push(other_file.container_id);
        }
    }
...
/// Check if a file's content references another file (for when we need to read the actual file)
fn check_file_references(content: &str, file_name: &str, module_path: &str) -> bool {
    ...
    let import_patterns = [
        format!(r#"use\s+.*{}"#, regex::escape(&file_name_lower)),
        format!(r#"use\s+.*{}"#, regex::escape(&module_path_lower)),
        format!(
            r#"from\s+['".]?{}['"]?\s+import"#,
            regex::escape(&file_name_lower)
        ),
        format!(
            r#"import\s+.*['"./]{}['"]"#,
            regex::escape(&file_name_lower)
    ...
    content_lower.contains(&format!("use {}", file_name_lower))
        || content_lower.contains(&format!("use {}", module_path_lower))
        || content_lower.contains(&format!("from {} import", file_name_lower))
        || content_lower.contains(&format!("import {}", file_name_lower))
        || content_lower.contains(&format!("require('{}')", file_name_lower))
        || content_lower.contains(&format!("require(\"{}\")", file_name_lower))
}
...
fn parse_structure(code: &str, language: &str) -> (Vec<FunctionInfo>, Vec<ClassInfo>) {
    ...
    let (fn_patterns, class_patterns): (Vec<&str>, Vec<&str>) =
        match language.to_lowercase().as_str() {
            ...
            "python" => (vec![r"(async\s+)?def\s+(\w+)"], vec![r"class\s+(\w+)"]),
            ...
            "go" => (vec![r"func\s+(\w+)"], vec![r"type\s+(\w+)\s+struct"]),
            "java" | "kotlin" => (
            ...
            if let Ok(re) = regex::Regex::new(pattern) {
                if let Some(caps) = re.captures(line) {
                    ...
                    if !name.is_empty() && name != "if" && name != "for" && name != "while" {
                        ...
                        let line_end = find_block_end(&lines, line_num) as u32;

                        // Extract function body and get calls
                        let function_body = extract_function_body(code, line_start, line_end);
                        let calls = extract_function_calls(code, language, &function_body);
...
fn extract_imports(code: &str, language: &str) -> Vec<ImportInfo> {
    ...
    let patterns: Vec<(&str, bool)> = match language.to_lowercase().as_str() {
        ...
        "go" => vec![(r#"import\s+["\(]([^"]+)["\)]"#, true)],
        _ => vec![(r"import\s+([\w.]+)", true)],
    };
...
fn extract_keywords(code: &str, language: &str) -> (Vec<String>, Vec<String>) {
    ...
    let topic_patterns = [
        ...
        (
            "networking",
            vec!["http", "fetch", "request", "socket", "tcp", "udp"],
        ),
        (
            "database",
            vec!["sql", "database", "query", "SELECT", "INSERT", "mongo"],
        ),
        (
            "cryptography",
            vec!["encrypt", "decrypt", "hash", "crypto", "sha", "md5"],
        ),
        (
            "serialization",
            vec!["json", "serialize", "deserialize", "parse", "stringify"],
        ),
        (
            "error_handling",
            vec!["error", "Error", "exception", "catch", "Result"],
        ),
        (
            "logging",
            vec!["log", "debug", "info", "warn", "error", "trace"],
        ),
        (
            "ui",
            vec!["render", "component", "view", "DOM", "React", "Vue"],
        ),
        ...
        (
            "concurrency",
            vec!["thread", "mutex", "channel", "spawn", "parallel"],
        ),
    ];
...
pub async fn execute(input: CodeAnalysisInput) -> Result<CodeAnalysisOutput, String> {
    match input {
        ...
        CodeAnalysisInput::FindSimilar { code, limit } => {
            ...
            // Use extended search that returns code content
            let query = serde_json::json!({
                "action": "SearchWithCode",
                "query": keywords.join(" "),
            });
            ...
            let mut similar: Vec<SimilarCode> = Vec::new();

            if let Some(containers) = result.get("containers").and_then(|c| c.as_array()) {
                for container in containers.iter().take(max_results * 2) {
                    // Get more, then filter by similarity
                    let container_id = match container.get("container_id").and_then(|c| c.as_u64())
                    {
                        Some(id) => id,
                        None => continue,
                    };

                    // Get the code content from the container
                    let candidate_code = container
                        .get("content")
                    ...
                    // FIX #2: Compute actual similarity instead of hardcoded 0.8
                    let similarity_score = if !candidate_code.is_empty() {
                        compute_similarity(&code, candidate_code)
                    } else {
                        // If no code content available, use keyword-based similarity
                        let candidate_keywords: HashSet<String> = container
                            .get("keywords")
                        ...
                        let intersection = candidate_keywords.intersection(&query_keywords).count();
                        let union = candidate_keywords.union(&query_keywords).count();
                    ...
                    if similarity_score >= 0.1 {
                        similar.push(SimilarCode {
                            container_id,
                            similarity_score, // Now computed, not hardcoded!
                            file_path: container
                                .get("file_path")
                                .or_else(|| {
                                    container.get("content").and_then(|c| c.get("file_path"))
                                })
                                .and_then(|p| p.as_str())
                                .map(|s| s.to_string()),
                            language: container
                                .get("language")
                                .or_else(|| {
                                    container.get("content").and_then(|c| c.get("language"))
                                })
                                .and_then(|l| l.as_str())
                                .unwrap_or("unknown")
                                .to_string(),
        ...
        CodeAnalysisInput::StoreAnalysis {
            analysis,
            project_id,
        } => {
        ...
        CodeAnalysisInput::GetDependencyGraph { project_id } => {
            ...
            // First pass: collect all files with their imports
            let mut all_files: Vec<FileInfoForDeps> = Vec::new();

            if let Some(files) = result.get("files").and_then(|f| f.as_array()) {
                for file in files {
                    let container_id = file
                        .get("container_id")
                        .and_then(|c| c.as_u64())
                        .unwrap_or(0);
                    let file_path = file
                        .get("file_path")
                        .or_else(|| file.get("content").and_then(|c| c.get("file_path")))
                        .and_then(|p| p.as_str())
                        .unwrap_or("")
                        .to_string();
                    ...
                    let imports: Vec<String> = file
                        ...
                        .map(|arr| {
                            arr.iter()
                                .filter_map(|i| i.get("module").and_then(|m| m.as_str()))
                                .map(|s| s.to_string())
                                .collect()
                        })
                        .unwrap_or_default();

                    all_files.push(FileInfoForDeps {
                        container_id,
                        file_path,
                        imports,
                    });
            ...
            // Second pass: build nodes with both depends_on and depended_by
            for file_info in &all_files {
                // Get dependencies (what this file imports)
                let depends_on: Vec<u64> = all_files
                    .iter()
                    .filter(|other| {
                        // Check if our imports reference this other file
                        file_info.imports.iter().any(|imp| {
                            let other_name = Path::new(&other.file_path)
                                .file_stem()
                                .and_then(|s| s.to_str())
                                .unwrap_or("");
                            imp.contains(other_name) || other.file_path.contains(imp)
                        })
                    })
                    .map(|f| f.container_id)
                    .collect();

                // FIX #3: Compute actual reverse dependencies
                let depended_by = compute_reverse_dependencies(&file_info.file_path, &all_files);

                nodes.push(DependencyNode {
                    container_id: file_info.container_id,
                    file_path: file_info.file_path.clone(),
                    depends_on,
                    depended_by, // Now properly populated!
                });
            }
        ...
        CodeAnalysisInput::BuildAndCheck {
            project_id,
            build_command,
        } => {
            ...
            let cmd = build_command.unwrap_or_else(|| match language {
